locals {
  ...
  boot_disk_size = coalesce(var.boot_disk.size, var.cluster_role == "control-plane" ? 50 : 100)
  ...
  extra_disks = concat(var.enable_longhorn ? [{
    cache_mode   = var.longhorn_disk.cache_mode
    interface    = "scsi1"
    size         = var.longhorn_disk.size
    ssd          = var.longhorn_disk.ssd
    storage_pool = var.longhorn_disk.storage_pool
  }] : [], coalesce(var.extra_disks, []))
}
